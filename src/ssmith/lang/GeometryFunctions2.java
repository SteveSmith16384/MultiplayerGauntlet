...
public class GeometryFunctions2 {
			...
			double fractionFrom0)
	{
		...
		double[] p = GeometryFunctions2.computePointOnLine (p0, p1, fractionFrom0);
	...
	public static void extendLine (double[] p0, double[] p1, double toLength,
			double anchor)
	{
		double[] p = GeometryFunctions2.computePointOnLine (p0, p1, anchor);

		double length0 = toLength * anchor;
		double length1 = toLength * (1.0 - anchor);

		GeometryFunctions2.extendLine (p, p0, length0);
		GeometryFunctions2.extendLine (p, p1, length1);
	}
	...
	public static void extendLine (double[] p0, double[] p1, double toLength)
	{
		double oldLength = GeometryFunctions2.length (p0, p1);
	...
	{
		double[] v = GeometryFunctions2.createVector (p0, p1);
		return length (v);
	}
	...
	{
		return GeometryFunctions2.length ((double) x0, (double) y0,
				(double) x1, (double) y1);
	}
	...
	public static double length (int[] x, int[] y, boolean isClosed)
	{
		...
		for (int i = 0; i < nPoints-1; i++)
			length += GeometryFunctions2.length (x[i], y[i], x[i+1], y[i+1]);

		// Add last leg if this is a polygon
		if (isClosed && nPoints > 1)
			length += GeometryFunctions2.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
			...
			int x, int y)
	{
		...
		double length = GeometryFunctions2.length (x0, y0, x1, y1);
		if (length == 0.0)
			return GeometryFunctions2.length (x0, y0, x, y);
		...
		length = GeometryFunctions2.length (xp, yp, x, y);
		return length;
	}
	...
	{
		double[] v0 = GeometryFunctions2.createVector (p0, p1);
		double[] v1 = GeometryFunctions2.createVector (p0, p2);

		double dotProduct = GeometryFunctions2.computeDotProduct (v0, v1);

		double length1 = GeometryFunctions2.length (v0);
		double length2 = GeometryFunctions2.length (v1);
	...
	public static boolean isLineIntersectingLine (int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3) {
		int s1 = GeometryFunctions2.sameSide (x0, y0, x1, y1, x2, y2, x3, y3);
		int s2 = GeometryFunctions2.sameSide (x2, y2, x3, y3, x0, y0, x1, y1);
			...
			int x1, int y1)
	{
		...
		if (GeometryFunctions2.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
				GeometryFunctions2.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
			return true;

		// If it intersects it goes through. Need to check three sides only.

		// Check against top rectangle line
		if (GeometryFunctions2.isLineIntersectingLine (lx0, ly0, lx1, ly1,
				x0, y0, x1, y0))
			return true;
			...
			int x1, int y1)
	{
		...
		if (GeometryFunctions2.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
			return true;
		...
		for (int i = 1; i < x.length; i++) {
			if (x[i-1] != x[i] || y[i-1] != y[i])
				if (GeometryFunctions2.isLineIntersectingRectangle (x[i-1], y[i-1],
						x[i], y[i],
						x0, y0, x1, y1))
					return true;
		}
			...
			int x1, int y1)
	{
		...
		if (n == 1)
			return GeometryFunctions2.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);

		//
		// If the polyline constituting the polygon intersects the rectangle
		// the polygon does too.
		//
		if (GeometryFunctions2.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
			return true;

		// Check last leg as well
		if (GeometryFunctions2.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
				x0, y0, x1, y1))
			return true;
		...
		// or separate.
		//
		if (GeometryFunctions2.isPointInsidePolygon (x, y, x0, y0) ||
				GeometryFunctions2.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
			return true;
	...
	public static double[] computePolygonCentroid (double[] x, double[] y)
	{
		...
		double area = GeometryFunctions2.computePolygonArea (x, y);
			...
			double[] intersection)
	{
		...
		double a0 = GeometryFunctions2.equals (x0, x1, LIMIT) ?
				INFINITY : (y0 - y1) / (x0 - x1);
		double a1 = GeometryFunctions2.equals (x2, x3, LIMIT) ?
		...
		if (GeometryFunctions2.equals (a0, a1)) {
			if (!GeometryFunctions2.equals (b0, b1))
				return -1; // Parallell non-overlapping

			else {
				if (GeometryFunctions2.equals (x0, x1)) {
					if (Math.min (y0, y1) < Math.max (y2, y3) ||
							Math.max (y0, y1) > Math.min (y2, y3)) {
						double twoMiddle = y0 + y1 + y2 + y3 -
								GeometryFunctions2.min (y0, y1, y2, y3) -
								GeometryFunctions2.max (y0, y1, y2, y3);
						y = (twoMiddle) / 2.0;
						x = (y - b0) / a0;
					}
					...
					if (Math.min (x0, x1) < Math.max (x2, x3) ||
						...
						double twoMiddle = x0 + x1 + x2 + x3 -
								GeometryFunctions2.min (x0, x1, x2, x3) -
								GeometryFunctions2.max (x0, x1, x2, x3);
						x = (twoMiddle) / 2.0;
						y = a0 * x + b0;
					}
					else return -1;
		...
		if (GeometryFunctions2.equals (a0, INFINITY)) {
			x = x0;
			y = a1 * x + b1;
		}
		else if (GeometryFunctions2.equals (a1, INFINITY)) {
			x = x2;
			y = a0 * x + b0;
		}
		else {
		...
		if (GeometryFunctions2.equals (x0, x1)) {
			if (y0 < y1)
				distanceFrom1 = y < y0 ? GeometryFunctions2.length (x, y, x0, y0) :
					y > y1 ? GeometryFunctions2.length (x, y, x1, y1) : 0.0;
					else
						distanceFrom1 = y < y1 ? GeometryFunctions2.length (x, y, x1, y1) :
							y > y0 ? GeometryFunctions2.length (x, y, x0, y0) : 0.0;
		}
		else {
			if (x0 < x1)
				distanceFrom1 = x < x0 ? GeometryFunctions2.length (x, y, x0, y0) :
					x > x1 ? GeometryFunctions2.length (x, y, x1, y1) : 0.0;
					else
						distanceFrom1 = x < x1 ? GeometryFunctions2.length (x, y, x1, y1) :
							x > x0 ? GeometryFunctions2.length (x, y, x0, y0) : 0.0;
		}

		double distanceFrom2;
		if (GeometryFunctions2.equals (x2, x3)) {
			if (y2 < y3)
				distanceFrom2 = y < y2 ? GeometryFunctions2.length (x, y, x2, y2) :
					y > y3 ? GeometryFunctions2.length (x, y, x3, y3) : 0.0;
					else
						distanceFrom2 = y < y3 ? GeometryFunctions2.length (x, y, x3, y3) :
							y > y2 ? GeometryFunctions2.length (x, y, x2, y2) : 0.0;
		}
		else {
			if (x2 < x3)
				distanceFrom2 = x < x2 ? GeometryFunctions2.length (x, y, x2, y2) :
					x > x3 ? GeometryFunctions2.length (x, y, x3, y3) : 0.0;
					else
						distanceFrom2 = x < x3 ? GeometryFunctions2.length (x, y, x3, y3) :
							x > x2 ? GeometryFunctions2.length (x, y, x2, y2) : 0.0;
		}

		return GeometryFunctions2.equals (distanceFrom1, 0.0) &&
				GeometryFunctions2.equals (distanceFrom2, 0.0) ? 1 : 0;
	}
			...
			double x1, double y1)
	{
		...
		for (int i = 0; i < nPoints; i++) {
			...
			if (GeometryFunctions2.equals (x2, x3) && GeometryFunctions2.equals (y2, y3))
				continue;

			int type = GeometryFunctions2.findLineSegmentIntersection (x0, y0, x1, y1,
					x2, y2, x3, y3,
					intersection);
			...
			if (type == -2) { // Overlapping
				...
				int side = GeometryFunctions2.sameSide (x0, y0, x1, y1,
						x[p1], y[p1], x[p2], y[p2]);
	...
	public static int[] createEllipse (int[] x, int[] y)
	{
		...
		double axisAngle = GeometryFunctions2.computeAngle (p0, p1, p2);

		// dx / dy  
		double dx = GeometryFunctions2.length (x0, y0, x[1], y[1]);
		double dy = GeometryFunctions2.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
		...
		// it to zero as well.
		if (!GeometryFunctions2.equals (axisAngle, Math.PI/2.0, 0.1) && 
				!GeometryFunctions2.equals (axisAngle, Math.PI,     0.1) &&
				!GeometryFunctions2.equals (axisAngle, 0.0,         0.1)) {
			...
			double length, double angle, double inset)
	{
		...
		int[] pos1 = new int[2];
		GeometryFunctions2.findPolygonPosition (x, y, length, pos1);
			...
			int[] position)
	{
		...
		for (int i = 1; i < x.length; i++) {
			double legLength = GeometryFunctions2.length (x[i-1], y[i-1], x[i], y[i]);
	...
	public static Point GetIntersectionPoint(float l1x, float l1y, float l1w, float l1h, float l2x, float l2y, float l2w, float l2h) {
		if (l1w == 1) {
			l1w = 1;
		}
		if (l1h == 1) {
		...
		if (isLineIntersectingLine(l1x, l1y, l1x+l1w, l1y+l1h, l2x, l2y, l2x+l2w, l2y+l2h) == false) {
			return null;
		}
		...
		if (det == 0) {
			...
			double z = (l2w*(l2y-l1y)+l2h*(l1x-l2x))/det;
			if (z==0 ||  z==1) {
				return null;  // intersection at end point!
			}
			return new Point((int)(l1x+z*l1w), (int)(l1y+z*l1h));
		}
	} // end intersection line-line
	
	
	public static void main(String args[]) {
		Point p = GetIntersectionPoint(1455, 386, 1, 5, 1441, 375, 28, 1);
		p = GetIntersectionPoint(0, 0, 10, 10, 0, 1, 10, 10);
		p = GetIntersectionPoint(1161, 929, 11, 0, 1155, 910, 0, 25);
